...
app.controller('charactersCtrl', function ($scope, $http, $q, $state) {
    ...
    $scope.blocks = blocks;
    var selectedBlocks = blocks.filter(function (block) { return block.startCode == params.start && block.endCode == params.end; });
    ...
    var refresh = $scope.refresh = function () {
        ...
        var matchingCharacters = characters.filter(function (character) {
...
function normalize(raw) {
    ...
    //    688 - 767
    var modifiers_applied = decompositions_applied.replace(/([\u02B0-\u02FF])(.)/g, function (m0, modifier, modified) {
...
function applyNormalization(form, input) {
    if (form == 'Original') {
        return input;
    }
    else if (form == 'Custom') {
...
app.controller('stringCtrl', function ($scope, $http, $state) {
    ...
    $scope.$watch('input', function (input) {
        ...
        $scope.normalizations = normalizations.map(function (form) {
...
function renderUcdTable(characters) {
    var rows = characters.map(function (character) {
        return h('tr', [
            ...
            h('td', [character.decompType, ' ', (character.decomp || []).map(function (code) { return '0x' + code.toString(16); }).join(' ')]),
...
function renderCharCodesTable(charCodes) {
    var indexCells = [h('th', '')];
...
},{"unidata":undefined,"unorm":3,"virtual-dom/create-element":4,"virtual-dom/diff":5,"virtual-dom/h":6,"virtual-dom/patch":14}],2:[function(require,module,exports){
